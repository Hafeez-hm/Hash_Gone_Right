...
run_for_lib_set() {
	...
	echo -e "\n########################################"
	echo "Starting simplified IKE benchmark with LIB_SET=$lib_set"
	echo "########################################"

	echo -e "\nSimplified ike with assoc: running..."
	time make -C "$ROOT_DIR" ike_s=1 assoc=1 LIB_SET="$lib_set" > "$log_dir/LOG_assoc.txt"
	grep RESULT "$log_dir/LOG_assoc.txt" || echo "No RESULT lines found"

	echo -e "\nSimplified ike no collision: running..."
	time make -C "$ROOT_DIR" ike_s=1 LIB_SET="$lib_set" > "$log_dir/LOG_no.txt"
	grep RESULT "$log_dir/LOG_no.txt" || echo "No RESULT lines found"

	echo -e "\nSimplified ike collision: running..."
	time make -C "$ROOT_DIR" ike_s=1 col=1 LIB_SET="$lib_set" > "$log_dir/LOG_col.txt"
	grep RESULT "$log_dir/LOG_col.txt" || echo "No RESULT lines found"

	echo -e "\nCompleted simplified IKE benchmark with LIB_SET=$lib_set"
}
...
find "$SCRIPT_DIR/logs" -type f -name "LOG_*.txt" | sort || echo "No simplified IKE log files found"